# 🛡️ SOC Lab – Security Operations Center Simulation

## 📌 Project Summary
This project is a hands-on simulation of a Security Operations Center (SOC) environment built using virtual machines.

It showcases how security events are created, detected, and managed through a full incident response process.

---

## 🚀 What This Project Shows

- Detection of real-world attack techniques  
- Log ingestion and analysis using Splunk 
- Incident response workflow with osTicket
- Active Directory security configuration using Windows 10 Server   
- MITRE ATT&CK–aligned attack simulations with AtomicRedTeam 

---

## 🔍 Key Demonstrations

- Brute force attack detection  
- Suspicious PowerShell activity  
- Privilege escalation (admin group changes)  
- New user account creation  
- Account lockout monitoring  

---

## 🎬 Demo Workflow

Each scenario follows a real SOC process:

1. Simulate an attack  
2. Generate logs  
3. Detect activity in SIEM  
4. Trigger alert  
5. Create ticket  
6. Investigate and resolve  

---

## 📂 Repository Structure

- `architecture/` → Network design and lab setup  
- `active-directory/` → OU structure, GPOs, security groups  
- `splunk/` → Alerts, dashboards, queries  
- `attack-simulations/` → Attack scenarios and results  
- `incident-response/` → Investigation process  
- `ticketing/` → osTicket setup and integration  

---

## 📖 Documentation

For full lab details, view:

➡️ `SOC/lab-overview.md`

---

## 🎯 Skills Demonstrated

- SIEM implementation  
- Log analysis and detection engineering  
- Active Directory administration  
- Incident response workflow  
- Security monitoring and alerting  

---

## 📎 Author

Bryce Falker
